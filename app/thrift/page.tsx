...
const categories = [
  { name: 'MENS JACKET', slug: 'mens-jacket' },
  { name: 'WOMENS JACKET', slug: 'womens-jacket' },
  { name: 'WOMENS JEANS', slug: 'womens-jeans' },
  { name: 'WOMENS TOP', slug: 'womens-top' },
]
...
export default function ThriftPage() {
  ...
  const loadProducts = useCallback(async () => {
    try {
      setIsLoading(true)
      const fetchedProducts = await fetchProductsByCategoryPattern('thrift')
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((p) => {
      // Check if subcategory matches (e.g., thrift-mens-jacket matches mens-jacket)
      if (p.subcategory === `thrift-${selectedCategory}` || p.subcategory === selectedCategory) {
        return true
      }
      return false
    })
      ...
      <div className="border-b border-gray-200 bg-base sticky top-0 z-40">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between mb-6">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 uppercase tracking-tight">THRIFT</h1>
