...
const categories = [
  ...
  { name: 'KOREAN OUTFIT', slug: 'korean-outfit' },
  { name: 'LIGHT OVERSHIRT', slug: 'light-overshirt' },
]
...
export default function MensPage() {
  ...
  const loadProducts = useCallback(async () => {
    try {
      setIsLoading(true)
      const fetchedProducts = await fetchProductsByCategoryPattern('mens')
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((p) => {
      // Check if subcategory matches (e.g., mens-jacket matches jacket)
      if (p.subcategory === `mens-${selectedCategory}` || p.subcategory === selectedCategory) {
        return true
      }
      return false
    })
      ...
      <div className="border-b border-gray-200 bg-base sticky top-0 z-40">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between mb-6">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 uppercase tracking-tight">MENS</h1>
          ...
          <div>
            ...
            {!isAdminLoading && isAdmin && (
              ...
              >
                <AddProductCard 
                  category="mens" 
                  subcategory={selectedCategory ? `mens-${selectedCategory}` : undefined} 
                />
              </motion.div>
            )}
          </div>
